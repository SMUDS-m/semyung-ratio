...
선 끝에 각 학과의 **최신 경쟁률 숫자**가 표시됩니다. 학과별 선택, 시점별 상세 값(마우스 올리기), 표 보기를 지원하고, 화면 폭에 따라 글자 크기가 자동으로 조절됩니다.
...
| 경쟁률 | 전 전형 합산 정원내 경쟁률 |
| 지원자 수 | 정원내 지원인원 |
| 작년 대비 | 올해 **일반전형** 경쟁률(실선) vs 작년 최종(점선) |
...
- **학생부교과 일반전형 단독** 값이라 전 전형 합산과 섞어 보면 안 됩니다. 그래서 "작년 대비" 모드에서는 올해 값도 일반전형 기준으로 맞춰 그립니다.
